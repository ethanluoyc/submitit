...
def parse_accounting_info(output: str) -> Dict[str, Dict[str, str]]:
    info = {}
    lines = output.split('\n')
    for line in lines:
...
class SgeInfoWatcher(core.InfoWatcher):
    ...
    def read_info(self, string: Union[bytes, str]) -> Dict[str, Dict[str, str]]:
        ...
        for part in parts:
            ...
            if not part_info:
                continue
            job_id = "{}_{}".format(part_info['jobnumber'], part_info["taskid"]) if part_info["taskid"].isnumeric() else part_info['jobnumber']
            all_stats[job_id] = {}
            state = "SUCCESS" if int(part_info['failed']) == 0 else "FAILED"
            all_stats[job_id]["State"] = state
        return all_stats

class SgeJob(core.Job[core.R]):

    watcher = SgeInfoWatcher(delay_s=60)


class SgeParseException(Exception):
    pass


class SgeJobEnvironment(job_environment.JobEnvironment):
...
class SgeExecutor(core.PicklingExecutor):
    """Sge job executor
    ...
    def _make_submission_file_text(self, command: str, uid: str) -> str:
        return _make_qsub_string(command=command, folder=self.folder, map_count=self.parameters['map_count'])
    ...
    def _internal_process_submissions(
        ...
        # scheduled as arrays.
        array_ex = SgeExecutor(self.folder, self.max_num_timeout)
        ...
        jobs: List[core.Job[tp.Any]] = [
            ...
            SgeJob(folder=self.folder, job_id=f"{first_job.job_id}_{a}", tasks=tasks_ids) for a in range(1, n + 1)
        ]
...
def _make_qsub_string(
    ...
    lines += ["export SUBMITIT_EXECUTOR=sge"]
    lines += [command]
    return "\n".join(lines)