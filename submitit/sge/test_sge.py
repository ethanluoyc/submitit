...
def mocked_slurm(state: str = "RUNNING", job_id: str = "12", array: int = 0) -> tp.Iterator[str]:
    with contextlib.ExitStack() as stack:
        ...
        envs = dict(_USELESS_TEST_ENV_VAR_="1", SUBMITIT_EXECUTOR="sge", SLURM_JOB_ID=str(job_id))
...
#         job: sge.SgeJob[None] = sge.SgeJob(tmp, "12")
...
def test_job_environment() -> None:
    with mocked_slurm(job_id="12"):
        assert job_environment.JobEnvironment().cluster == "sge"


# def test_slurm_job_mocked() -> None:
#     with mocked_slurm() as tmp:
#         executor = sge.SgeExecutor(folder=tmp)
...
#     monkeypatch.setenv("_TEST_CLUSTER_", "sge")
...
#         "submitit.sge.sge.SgeJobEnvironment._requeue", return_value=None
#     ):
...
#     string = sge._make_sbatch_string(
#         command="blublu",
#         folder="/tmp",
#         partition="learnfair",
#         exclusive=True,
...
#     string = sge._make_sbatch_string(command="blublu", folder="/tmp", gpus_per_node=2)
...
# """
#     output = sge.SgeInfoWatcher().read_info(example)
...
def test_parse_accounting_info():
    info = sge.parse_accounting_info(_TEST_QACCT_OUTPUT)
...
def test_name() -> None:
    assert sge.SgeExecutor.name() == "sge"
